...
\begin{document}
    ...
    The problem of deep learning is the amount of data that is required to train them effectively.
    ...
    Nowadays with the use of tools as Segment Anything~\cite{kirillov2023segment} which allows segmenting any type of image based on priors, the labelling time would be much shorter.
    ...
    We have images of the whole earth if we can have enough images with ground truths as in figure~\ref{fig:tile-segmented} we could train a system to segment this bodies in
    each tile so as the satellite is moving around the earth an automated system could run when these tiles are being generated in real-time.
    ...
    Other semantic segmentation datasets as Postdam~\cite{postdam_dataset} provide high resolutionimages, 5cm per pixel of
    city areas.
    ...
    Some of them as BigEarthNet or EuroSAT include multispectral data as they come from Sentinel-2, but others like PatterNet or Million-AID are restricted to RGB data.
    ...
    The labels are rectangles where the area has or does not have vegetation floating in water.
    The labelled data consists in a table with the ROI being defined by the corner coordinates in WKT format, the label and the date
    of the image where it was located.
    An example of two images can be seen at figure~\ref{fig:vegetation_example}.
    This dataset has been captured manually by the researchers at \textcolor{red}{Universidad de Valencia. IS IT?!}
    ...
    \begin{figure}[h]
        ...
        In red, the rectangle with the coordinates where the label has been asigned.}
        \label{fig:vegetation_example}
    \end{figure}


    \section{Approach}
    In this work we use a self-supervised learning (SSL from now) architecture as they have shown to generate strong baseline models.
    ...
    The model then it is trained for 20 epochs or until the loss stabilizes.
    ...
    This technique allows to easily visualize embeddings of high dimensional data as it cluster together data with similar values while it moves away
    ...
    \subsection{Eurosat finetuning}

    \subsection{Eurosat image retreival}

    \subsection{Custom vegetation dataset embeddings visualization}

    \subsection{Custom vegetation dataset finetuning}

    \subsection{Custom vegetation dataset image retreival}
